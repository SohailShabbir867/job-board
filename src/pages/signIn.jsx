...
export default function SignIn() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const user = loginUser(formData.email.trim(), formData.password);

      // you could choose sessionStorage if remember is false
      // for now we always keep localStorage (demo)
      setLoading(false);
      navigate("/");
    } catch (err) {
      setLoading(false);
      ...
      <div className="w-full" style={{ width: '99%', maxWidth: 420, margin: '0 auto' }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <label className="inline-flex items-center gap-2 text-sm text-gray-300">
              <input
                ...
                onChange={handleChange}
                className="w-4 h-4 bg-[#071026] text-[#2563EB]"
              />
              Remember me
            </label>
            <Link to="/register" className="text-sm text-[#60a5fa] hover:underline">Create account</Link>
           </div>
         ...
         <div className="mt-6 text-center text-sm text-gray-300">
           By continuing you agree to this demo site's terms. (Demo only.)
         </div>
        </div>
      </div>
    </div>
